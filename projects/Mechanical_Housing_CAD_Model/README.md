# Mechanical Housing CAD Model

## Preview

![Preview](../../previews/Mechanical_Housing_CAD_Model_Preview.PNG)

## Overview

Mechanical housing modeled in SolidWorks from a 2D engineering drawing. The project demonstrates the complete workflow from sketching and feature creation to technical drawing preparation.

## Features Used

* Revolved Boss/Base
* Extruded Cut
* Hole Wizard
* Linear Pattern
* Fillet
* Section View
* Technical Drawing Creation
* Engineering Dimensions and Annotations

## Technical Specifications

* Material: Structural Steel (S235)
* Overall Length: 154 mm
* Maximum Diameter: 127.47 mm
* Central Bore Diameter: 30 mm

## Project Gallery

## Section View

![Section View 1](section-view-1.png)

![Section View 2](section-view-2.png)

![Section View 3](section-view-3.png)

### Technical Drawing

![Drawing](projects/Mechanical Housing CAD Model/Mechanical_Housing_CAD_Model_Drawing.jpg)

## Skills Demonstrated

* Reading engineering drawings
* Parametric CAD modeling
* Creation of repetitive geometry using patterns
* Mechanical part detailing
* Preparation of manufacturing drawings
* CAD visualization and presentation

## Software

* SolidWorks 2025 Premium

## Files

* SolidWorks Part (.SLDPRT)
* SolidWorks Drawing (.SLDDRW)
* PDF Drawing
* Render Images
